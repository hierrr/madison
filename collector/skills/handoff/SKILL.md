---
name: handoff
description: 현재 작업을 다른 기기로 이관한다 — WIP 커밋·push 후 핸드오프 문서를 작성하고 MADISON 허브 큐에 등록. 사용법 /handoff <대상기기> [한 줄 요약]. 대상 기기에서 세션을 열면 자동으로 주입된다.
---

# /handoff — 작업을 다른 기기로 이관

원리: **코드 상태는 git이, 의도·맥락은 문서가, 수신 보장은 대상 기기의 SessionStart 훅이** 담당한다.
...
5. **허브 큐 등록**:
   ...
   ```
6. **보고**: 응답의 HF-ID와 함께 "대상 기기에서 이 리포의 세션을 열면 자동 주입되고, 즉시 가져오려면 /pickup"이라고 사용자에게 알린다.
