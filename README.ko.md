...
다른 기기에서 접근하는 구성은 두 갈래인데, **기본은 내부망**입니다. 터널은 망 밖에서
허브에 닿아야 하는 기기가 있을 때만 더하면 됩니다:

- **내부망** (기본 — 터널 없음): `HOST=0.0.0.0`으로 열고 터널 키는 비워둡니다. 수집기는
...
- **터널** (선택 — 외부 접근): 허브를 터널 뒤에 둡니다 — 예: Cloudflare Tunnel. `.env`의
  호스트명과 `CF_ACCESS_*` 키가 이 구성용입니다. 사람용 대시보드 호스트는 SSO로
  보호하고, 기계용 API 호스트는 기기별 토큰으로 인증하는 식입니다. `.env.example`을
  참고하세요. 터널이 내부망 경로를 대체하는 건 아닙니다 — 허브와 같은 망의 기기는
  계속 내부망 주소로 보고하고, 터널 호스트는 망 밖의 기기용입니다.
...
```bash
curl -fsSL http://<허브IP>:8787/install.sh | bash -s -- \
  --name studio --secret <ENROLL_SECRET> --hub http://<허브IP>:8787
# Claude Code + Codex 수집이 모두 기본 — Codex를 빼려면 --no-codex
```

`--hub`는 이 기기가 앞으로 보고를 보낼 주소이고, **첫 설치에는 필수**입니다 —
설치 스크립트가 터널 호스트를 기본값으로 조용히 잡는 일은 없습니다. 내부망 주소를
기본으로 쓰고, 터널 호스트(`--hub https://madison-api.example.com`)는 망 밖에서
보고하는 기기 — 예컨대 밖으로 들고 다니는 노트북 — 에만 넘기세요. 설치를 그 기기의
에이전트에게 맡길 때는 `--hub` 값을 명령에 박아서 건네는 게 가장 확실하지만, 빠뜨려도
설치 스크립트의 에러 문구와 완료 안내가 에이전트를 내부망 우선으로 이끌고, 애매하면
(기기가 망 밖으로 나가는지) 사용자에게 확인하라고 지시합니다. 설치가 끝나면 이 기기가
어느 경로(내부망 직결/터널)로 보고하는지와 전환 방법을 요약해 주며, 다른 `--hub`로
재실행하면 재등록 없이 주소만 바뀝니다.
