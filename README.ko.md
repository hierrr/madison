...
- **일일/주간/월간 리포트** — 허브가 기간별 작업을 PM/오너 관점의 보고용 마크다운
  ...
  주간·월간은 원본 로그가 아니라 저장된 일일 일지를 종합합니다. 서비스 묶음은 모델 추측이 아니라
  `REPORT_SERVICE_MAP` 설정으로 고정되고, 항목은 **작업 대상** 기준으로 들어갑니다 —
  다른 저장소를 열어둔 채 발견한 허브 결함은 그 저장소가 아니라 허브 밑에 놓입니다. 주기 자동 갱신에 수동 갱신도 되고, 사용
  메트릭(턴·세션·활동 시간·프로젝트별/시간대 분포·52주 스트릭 잔디 — 월 표시,
  좌우 스크롤, 처음엔 최신 주가 보임)도 함께 보여줍니다.
