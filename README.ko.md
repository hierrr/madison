...
- **일일/주간/월간 리포트** — 허브가 기간별 작업을 PM/오너 관점의 보고용 마크다운
  ...
  작업 제목은 맥락으로)으로 읽고 전날 일지의 주제를 이어받아, 후속 질문이 원래 과제
  아래에 들어갑니다. 잘라내지 않습니다 — 너무 긴 세션은 버리는 대신 먼저 압축합니다.
  ...
  별도 워커 프로세스에서 하고 수동 갱신도 됩니다. 생성이 실패해도 기존 리포트를 덮지 않으며,
  리포트를 고정할 수 있습니다. 사용 메트릭(턴·세션·활동 시간·서비스별/프로젝트별/시간대 분포·
...
쓰면 됩니다 — 예를 들어 허브 기기의 에이전트에게 시키는 식으로. 생성본은 리포트마다 최근 5개가 보관되고
(`report_versions`, `POST /api/report/restore`로 복원), 복원하거나 고정한 리포트는 자동 재생성에서 제외됩니다.
고정은 리포트 탭의 버튼입니다.
