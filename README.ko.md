...
- **일일/주간/월간 리포트** — 허브가 기간별 작업을 PM/오너 관점의 보고용 마크다운
  ...
  나열 대신 더 포괄적으로 종합합니다. 일일은 하루를 세션 흐름(지시→응답 짝, 백그라운드
  작업 알림은 제목·본문 발췌를 맥락으로)으로 읽고 전날 일지의 주제와 진행 중이던 세부를
  이어받아, 후속 질문이나 밤새 돈 배치의 알림이 원래 과제 이름 아래에 들어갑니다.
  자동화가 붙인 내부 라벨은 과제명이 되지 않습니다: 라벨만 남은 날은 과거 일지를 검색해
  그 라벨이 가리키던 과제 이름을 찾아 쓰고(별도 기억 저장소 없이 일지 아카이브가 곧
  작성기의 기억), 라벨 유래 과제명은 검증기가 잡아 재명명시킵니다. 잘라내지 않습니다 —
  너무 긴 세션은 버리는 대신 먼저 압축합니다.
  주간·월간은 원본 로그가 아니라 저장된 일일 일지를 종합합니다. 최상위 묶음은 작성기가 스스로
  유지하는 서비스 레지스트리(`.env`로 시드, 아래 참고)를 쓰고, 항목은 **작업 대상** 기준으로
  들어갑니다 — 다른 저장소를 열어둔 채 발견한 허브 결함은 그 저장소가 아니라 허브 밑에 놓입니다.
  모든 리포트는 저장 전에 결정적 검증기를 통과합니다. 갱신은 크론 시각에 바뀐 것이 있을 때만
  별도 워커 프로세스에서 하고 수동 갱신도 됩니다. 생성이 실패해도 기존 리포트를 덮지
  않습니다. 사용 메트릭(턴·세션·활동 시간·서비스별/프로젝트별/시간대 분포·
  52주 스트릭 잔디 — 월 표시, 좌우 스크롤, 처음엔 최신 주가 보임)도 함께 보여줍니다.
- **구독 사용량** — 현황에 Claude Code·Codex의 한도 창(5시간·주간·초기화 크레딧)을 바와
  경고색으로 보여줍니다. 허브가 프로바이더의 엔드포인트에서 직접 읽습니다(보안 모델 참고).
  게이지는 **허브 기기에 로그인된 계정 기준**입니다 — 한도는 계정 단위라 같은 계정을 쓰는
  모든 기기의 소비가 이미 합산돼 있고, 다른 계정으로 로그인한 기기의 한도는 나오지 않습니다
  (토큰 집계는 계정과 무관하게 기기별로 전부 잡힙니다). 허브에 해당 계정이 없으면 그
  프로바이더의 게이지·한도 차트는 빈 채로 남지 않고 숨겨집니다.
- **사용량 탭 — 토큰 집계·한도 이력** — 수집기가 에이전트의 로컬 전사본에서 세션 누적
  토큰(입력/출력/캐시 읽기/캐시 쓰기/생각, 모델별)을 읽어 턴 이벤트에 실어 보내고, 허브는
  일 × 기기 × 프로젝트 × 모델 원장으로 접어 영구 보존합니다. 한도 퍼센트도 변할 때마다
  이력으로 남습니다. 탭에서는 한도 창 계단선 차트, 일별 토큰 스택(입출력과 캐시는 축 분리),
  서비스/프로젝트/모델/기기/에이전트/세션별 내역을 기간·에이전트·기기·자동화 필터로
  봅니다. 디스크에 남은 과거 전사본은 일회성 스크립트로 백필합니다.
- **허브 LLM 선택** — 설정 탭에서 태스크 요약·세션 압축·리포트 생성 각각의 프로바이더
...
- **Hub**(한 대): FastAPI 프로세스 하나가 JSON API와 대시보드를 함께 서빙하고, 그 뒤를
  ...
  리포트 생성은 허브가 띄우는 짧은 워커 프로세스에서 돌아, 허브 자체는 언제든 재시작할 수 있습니다.
...
구독 한도 게이지를 쓰려면 **허브 기기에서 Claude Code에 로그인**돼 있어야 합니다(macOS는
최초 조회 때 키체인 접근 허용 팝업이 한 번 뜰 수 있음 — GUI 세션에서 허용하면 끝). Codex
게이지는 허브에 codex CLI가 로그인돼 있을 때 나옵니다. 둘 다 없어도 나머지 기능은 전부
정상이고 게이지만 숨겨집니다.
...
macOS 수집기보다 기능이 뒤처지는** 베타입니다(예: 백그라운드 서브에이전트·셸 카운터 미구현). 쓰는 방식은 똑같은 제로터치 온보딩이라, 그
...
## 서비스 레지스트리 (리포트 최상위 이름)

리포트는 저장소가 아니라 *서비스* 단위로 묶입니다. 서비스 이름 집합은 허브 DB(`services`, `project_map`)에 있고
리포트 작성기가 스스로 유지합니다 — 설정 UI는 없습니다:

- 일일 리포트는 **구조화 출력 1회 호출**(`{markdown, assignments, proposals}`)입니다. 최상위 불릿은 레지스트리의
  이름이어야 하고, 새 이름이 필요하면 작성기가 근거와 함께 제안하며 허브가 등록합니다(가드: 저장소·디렉터리 이름,
  제외 이름, 사람이 거절했던 이름은 거부, 하루 3건 상한).
- **프로젝트 → 서비스 매핑**과 *강도*: 제품 저장소는 `strong`, 잡동사니 디렉터리는 `weak`(실제 대상 서비스로
  옮겨짐). 매핑 없는 프로젝트의 세션이 한 서비스로 일관되게 배치되면 약한 매핑으로 자동 학습합니다.
- 결정적 검증기가 마크다운을 검사합니다(허용된 최상위 이름·들여쓰기·헤더 없음·잘림/메타 문구 없음·제외 이름 없음).
  위반은 수정 호출 1회, 남는 것은 리포트에 *검토 필요*로 표시됩니다.
- 세션별 배치(서비스·과제·근거)와 사람의 교정은 저장되며, API(`POST /api/report/relabel`)로 넣은 교정은 이후
  생성에 사례로 주입되어 작성기의 단서보다 우선합니다.

`.env`의 `REPORT_SERVICE_MAP` / `REPORT_KNOWN_SERVICES` / `REPORT_WEAK_PROJECTS`는 레지스트리가 비어 있을 때 한 번
시드로만 읽습니다. 조정이 필요하면 관리자 API(`/api/services`, `/api/project-map`, 백업용 `/api/services/export`)를
쓰면 됩니다 — 예를 들어 허브 기기의 에이전트에게 시키는 식으로. 생성본은 리포트마다 최근 5개가 보관됩니다
(`report_versions`, `POST /api/report/restore`로 복원).

## 보안 모델

- **세션 전문은 기기 밖으로 나가지 않습니다.** 허브에는 메타데이터와 짧게 잘린 발췌만
  남습니다 — 지시문 앞 약 600자(요약용), 턴의 마지막 응답 최대 2,000자(더 길면 머리 1,500 + 꼬리 500 —
  리포트에 필요한 것은 결론과 남은 일이지 중간이 아님), 승인 메시지 200자, 그리고 경로 수준 메타데이터
  (저장소 이름·브랜치·git 원격 URL·저장소 안 작업 디렉터리 경로)입니다. 파일 내용은 나가지 않습니다. 의도된 예외는
  핸드오프 하나입니다: `/handoff`는 사용자의 명시적 지시로 문서와 변경 diff를 허브에
  올립니다(64KB / 1MB 상한).
- **리포트 생성은 별도 워커 프로세스**(`python -m server.genworker`)에서 돕니다. 허브를 재시작·재배포해도 진행 중인
  LLM 호출이 버려지지 않고, 허브는 워커를 띄우고 상태(`report_jobs`)만 보여 줍니다. 워커는 한 번에 하나.
- **허브의 LLM 호출은 격리됩니다.** `claude -p`는 `--safe-mode --tools "" --no-session-persistence
  --disable-slash-commands`(codex는 `--ephemeral -s read-only`)로 저장소 밖 디렉터리에서 실행되어, 사용자의
  CLAUDE.md·플러그인·훅·MCP·스킬이 요약기나 리포트 작성기에 들어가지 않고 세션 전사본도 남기지 않습니다.
  모든 호출은 `llm_runs`에 기록되어(사이트·모델·소요·종료 코드) `GET /api/llm-runs`로 볼 수 있습니다.
...
| `DIGEST_MODEL` | `claude-sonnet-5` | 프로젝트 블록이 예산을 넘을 때 긴 세션을 미리 압축하는 모델 — 추출 작업이라 리포트 모델보다 싼 모델로 충분 |
| `LLM_TIMEOUT_SUMMARY` / `LLM_TIMEOUT_DIGEST` / `LLM_TIMEOUT_REPORT` | `90` / `300` / `900` | 허브 LLM 호출의 사이트별 타임아웃(초). 실패·타임아웃은 저장된 리포트를 덮지 않고 사유만 기록·표시 |
| `LLM_CWD` | `~/.madison/llm-cwd` | 허브 LLM 호출의 작업 디렉터리 — 저장소 밖이라 CLI가 git 컨텍스트를 보지 않음 |
| `USAGE` / `USAGE_POLL_SEC` | `1` / `180` | 현황의 Claude / Codex 구독 한도 타일. 허브가 직접 수집 — Claude는 Claude Code가 키체인에 두는 OAuth 토큰으로 읽기 전용 usage 엔드포인트 조회(토큰 저장 안 함), Codex는 `codex app-server` RPC. `USAGE=0`이면 숨김 |
| `REPORT_DAILY_CRON` / `REPORT_WEEKLY_CRON` / `REPORT_MONTHLY_CRON` | `0 * * * *` / `15 */4 * * *` / `45 */8 * * *` | 각 리포트를 다시 만드는 로컬 시각(크론 5필드) — 마지막 판 이후 바뀐 것이 있을 때만. 새 날·주·월의 첫 리포트는 즉시 만들고, 허브 재시작은 생성을 유발하지 않음. 생성은 별도 워커 프로세스에서 |
| `REPORT_EXCLUDE_PROJECTS` | *(없음)* | 리포트에서 제외할 프로젝트(콤마 구분) — 해당 프로젝트 섹션과, 다른 프로젝트 로그에서 그 이름이 언급된 줄까지 뺀다 |
| `REPORT_SERVICE_MAP` | *(없음)* | 리포트 최상위 묶음이 될 `프로젝트=서비스` 쌍(콤마 구분, 예: `web=에이크미, api=에이크미`). 매핑 없는 프로젝트는 프로젝트명 그대로 쓰이고, 서비스명이 다른 프로젝트는 절대 합쳐지지 않는다 |
| `REPORT_WEAK_PROJECTS` | *(없음)* | 잡동사니 디렉터리(콤마 구분, 예: `dev,new-chat`) — 이들의 `REPORT_SERVICE_MAP` 매핑은 *약한 기본값*이라 실제 작업 대상 서비스로 옮겨집니다 |
| `REPORT_KNOWN_SERVICES` | *(없음)* | 자기 프로젝트가 없는 서비스(다른 서비스 저장소·모노리포 안에서 작업되는 경우)를 `서비스=식별 단서`로 세미콜론 구분해 등록. 리포트 프롬프트의 '알려진 서비스' 목록에 단서와 함께 실려, 그 서비스 대상 항목이 로그가 붙은 서비스가 아니라 올바른 서비스 밑으로 들어간다 (예: `에이크미 프로=에이크미 모노리포 안에서 작업됨, ap- 접두 화면, 결제 메뉴`) |
...
| 경로 | 내용 |
|---|---|
| `server/` | 허브 — FastAPI + SQLite. `app.py` 라우트 · `state.py` 수신/폴드/TTL · `llm.py` CLI 호출 · `summary.py` 작업 요약 · `report.py` 리포트 재료/프롬프트/검증기 · `reporting.py` + `genworker.py` 생성 워커 · `cron.py` 스케줄 · `registry.py` 서비스 이름 · `usage.py` 구독 한도 |
